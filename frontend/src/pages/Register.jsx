...
const Register = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 text-white p-4 overflow-hidden relative">
      <div className="absolute top-0 right-0 w-[32rem] h-[32rem] bg-indigo-600/20 rounded-full blur-[100px] pointer-events-none"></div>
      <div className="absolute bottom-0 left-0 w-[24rem] h-[24rem] bg-cyan-600/20 rounded-full blur-[100px] pointer-events-none"></div>
      ...
      <div className="w-full max-w-md p-8 rounded-3xl bg-white/5 backdrop-blur-2xl border border-white/10 shadow-2xl relative z-10 transition-all hover:border-white/20 duration-500">
        ...
        <p className="text-slate-400 text-center mb-8 font-light text-sm">
          {inviteToken ? 'You have been invited! Register to join the household.' : 'Join the AI-powered utility platform.'}
        </p>
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          <div className="relative group">
            <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
              <User className="h-5 w-5 text-indigo-300/50 group-focus-within:text-cyan-400 transition-colors" />
            </div>
            <input 
              type="text" required
        ...
        <p className="mt-8 text-center text-sm text-slate-400">
          Already have an account? <Link to="/login" className="text-cyan-400 hover:text-cyan-300 font-medium transition-colors ml-1">Sign in</Link>
        </p>
      </div>
    </div>
  );
